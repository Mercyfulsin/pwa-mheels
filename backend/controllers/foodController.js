...
module.exports = {
  ...
  findAllWithName: function (req, res) {
    const foodName = req.query.foodName;
    let condition = foodName ? {
      foodName: {
        [Op.like]: `%${foodName}%`
      }
    } : null;
  ...
  deleteAll: function (req, res) {
    Food.destroy({
      ...
      .then(nums => {
        res.send({
          message: `${nums} Food were deleted successfully!`
        });
      })
      .catch(err => {
        res.status(500).send({
          message: err.message || "Some error occurred while removing all food items."
        });
      });
  },
  // Find all popular Food
  findAllPopular: function (req, res) {
    Food.findAll({
        where: {
          popular: true
        }
